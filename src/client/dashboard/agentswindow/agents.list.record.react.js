...
class AgentListRecord extends Component {
  ...
  render() {
    const {agent, agentbeingsaved, agentondisplay, agentsinroster, debug, self} = this.props;
    ...
    return (
      <tr
        ...
        <td>
          {(agent.get('prison') || debug) && (agentsinroster.indexOf(agent) !== -1) && (agent.get('id') !== self.get('id')) &&
            <button
